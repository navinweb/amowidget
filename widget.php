...
class Widget extends \Helpers\Widgets {
	...
	protected function endpoint_set() {
			...
			'custom_fields'   => array(
				array(
					...
					'values' => array(
						...
						array(
							'value' => '7(' . rand( 0, 999 ) . ')' . rand( 0, 999 ) . '-' . rand( 0,
									99 ) . '-' . rand( 0, 99 ),
							'enum'  => 'HOME',
						),
					),
				...
				array(
					...
					'values' => array(
						0 => array(
							'value' => str_shuffle( 'qwertyuiopasdfghjklzxcvbnm' ) . ' ' . rand( 0,
									999999 ) . str_shuffle( 'qwertyuiopasdfghjklzxcvbnm' )
						)
					)
				),
				array(
		...
		$request['update'][] = array(
			'id'                  => 1232810,
			'last_modified'       => time(),
			//if last modified is lower than in amoCRM DB, then it will not rewrite, but will return what it have
		...
		$request = array(
			'add'    => array(
				array(
					'element_id'    => 8000651,
					//contact/lead id
					'element_type'  => $this->_types['contacts'],
					//contact/lead type - can be found in variable $this->_types['contacts'] or $this->_types['leads']
					//'date_create'=>time()-360000, //optional field
					//'last_modified'=>time(), //optional field
					'task_type'     => 2233,
					//can be found in $this->account->current('task_types'), can be CALL,LETTER,MEETING or id of task_type
		...
		$request = array(
			'add'    => array(
				array(
					...
					'status_id'     => 26267,
					//optional - default first account status - list of statuses $this->account->current('leads_statuses')
					'price'         => 500000,
					'custom_fields' => array(
						array(
							'id'     => 290642,
							'values' => array(
								0 => array(
									'value' => str_shuffle( 'qwert\'"yuiopasdfghjklzxcvbnm' ) . ' ' . rand( 0,
											999999 ) . str_shuffle( 'qwertyuiopasdfghjklzxcvbnm' )
								)
							)
						),
